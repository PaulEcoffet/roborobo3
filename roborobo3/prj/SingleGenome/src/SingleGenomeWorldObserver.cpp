...
SingleGenomeWorldObserver::SingleGenomeWorldObserver(World *world) : WorldObserver(world)
{
    ...
    gProperties.checkAndGetPropertyValue("gNbNeuronsPerHiddenLayer", &SingleGenomeSharedData::gNbNeuronsPerHiddenLayer,
                                         true);
    ...
    gProperties.checkAndGetPropertyValue("gMaxNbGenomeTransmission", &SingleGenomeSharedData::gMaxNbGenomeTransmission,
                                         true);
    gProperties.checkAndGetPropertyValue("gLimitGenomeTransmission", &SingleGenomeSharedData::gLimitGenomeTransmission,
                                         true);
    gProperties.checkAndGetPropertyValue("gSelectionMethod", &SingleGenomeSharedData::gSelectionMethod, true);

    gProperties.checkAndGetPropertyValue("gNotListeningStateDelay", &SingleGenomeSharedData::gNotListeningStateDelay,
                                         true);
    ...
    gProperties.checkAndGetPropertyValue("gIndividualMutationRate", &SingleGenomeSharedData::gIndividualMutationRate,
                                         false);
...
//  gProperties.checkAndGetPropertyValue("gGenerationLog", &SingleGenomeSharedData::gGenerationLog, false);
    ...
    {
        std::cout << "Error: gInitialNumberOfRobots (" << gInitialNumberOfRobots
                  << ") should be equal to 1. Exiting.\n\n";
        exit(-1);
    }
...
void SingleGenomeWorldObserver::reset()
{
    ...
    if (SingleGenomeSharedData::gFakeCoopSteps > 1)
    {
        for (int i = 0; i < SingleGenomeSharedData::gFakeCoopSteps; i++)
            _fakeCoopValues.push_back(((double) i) / ((double) (SingleGenomeSharedData::gFakeCoopSteps - 1)) *
                                      SingleGenomeSharedData::gFakeCoopValue);
    }
    else
    {
    ...
    gMaxIt = SingleGenomeSharedData::gEvaluationTime * nbGenomes * SingleGenomeSharedData::gNbReplicas *
             SingleGenomeSharedData::gFakeCoopSteps * (SingleGenomeSharedData::gFakeRobotsPerObject + 1);
...
void SingleGenomeWorldObserver::stepEvaluation()
{
    if (SingleGenomeSharedData::gOnlyOneRobot)
        printf("Genome %d fakeCoop %.2d (%.2lf) fakeRobots %d rep %.2d done!\n", _genome, _fakeCoop,
               _fakeCoopValues[_fakeCoop], _nbFakeRobots, _replica);
    else
