...
void
TemplateEEController::step() // handles control decision and evolution (but: actual movement is done in roborobo's main loop)
{
    _iteration++;
    ...
    if (_wm->isAlive() == false)
    {
        ...
        else if (_notListeningDelay != -1 && _listeningDelay > 0)
        {
    ...
    for (int i = 0; i < _wm->_cameraSensorsNb; i++)
    {
        ...
        if (gExtendedSensoryInputs) // EXTENDED SENSORY INPUTS: code provided as example, should be rewritten to suit your need.
        {
            ...
            if (Agent::isInstanceOf(objectId))
            {
                ...
                // same group?
                if (gWorld->getRobot(objectId - gRobotIndexStartOffset)->getWorldModel()->getGroupId() ==
                    _wm->getGroupId())
                {
                ...
                double tgtOrientation = gWorld->getRobot(
                        objectId - gRobotIndexStartOffset)->getWorldModel()->_agentAbsoluteOrientation;
                ...
                else if (delta_orientation <= -180.0)
                    delta_orientation = -(-360.0 - delta_orientation);
            ...
            // input: wall or empty?
            if (objectId >= 0 &&
                objectId < gPhysicalObjectIndexStartOffset) // not empty, but cannot be identified: this is a wall.
                inputs.push_back(1);
            else
...
void TemplateEEController::stepEvolution()
{
    ...
    else
    {
        ...
        _dSumTravelled = _dSumTravelled + getEuclideanDistance(_wm->getXReal(), _wm->getYReal(), _Xinit,
                                                               _Yinit); //remark: incl. squareroot.
    }

    // log the genome (only at the second iteration during snapshot time)
    if (TemplateEESharedData::gLogGenomeSnapshot &&
        gWorld->getIterations() % (TemplateEESharedData::gEvaluationTime * TemplateEESharedData::gSnapshotsFrequency) ==
        1)
    {
        // Logging: full genome
        std::string sLog = std::string("");
...
{
    if (TemplateEESharedData::gIndividualMutationRate >
        ...
        switch (TemplateEESharedData::gMutationOperator)
        {
            ...
            default:
                std::cerr << "[ERROR] unknown variation method (gMutationOperator = "
                          << TemplateEESharedData::gMutationOperator << ")\n";
                exit(-1);
        }
    }
}
 ...
 */
bool TemplateEEController::storeGenome(std::vector<double> genome, std::pair<int, int> senderId, float sigma,
                                       float fitness) // fitness is optional (default: 0)
{
    if (!_isListening)
    {
    ...
    else
    {
        ...
        */
        ...
        if (it !=
            _genomesList.end()) // this exact agent's genome is already stored. Exact means: same robot, same generation. Then: update fitness value (the rest in unchanged)
        {
...
void TemplateEEController::setIOcontrollerSize()
{
    ...
    {
        _nbInputs = (PhysicalObjectFactory::getNbOfTypes() + 3 + 1) *
                    _wm->_cameraSensorsNb; // nbOfTypes + ( isItAnAgent? + isItSameGroupId? + agentAngleDifference?) + isItAWall?
    }
...
void TemplateEEController::initController()
{
    ...
    {
        _currentGenome.push_back((double) (randint() % TemplateEESharedData::gNeuronWeightRange) /
                                 (TemplateEESharedData::gNeuronWeightRange / 2) -
                                 1.0); // weights: random init between -1 and +1
    }
...
void TemplateEEController::broadcastGenome()
{
    ...
    for (int i = 0; i < _wm->_cameraSensorsNb && (TemplateEESharedData::gLimitGenomeTransmission == false ||
                                                  (TemplateEESharedData::gLimitGenomeTransmission == true &&
                                                   _nbGenomeTransmission <
                                                   TemplateEESharedData::gMaxNbGenomeTransmission)); i++)
    {
        int targetIndex = _wm->getObjectIdFromCameraSensor(i);

        if (targetIndex >= gRobotIndexStartOffset && targetIndex < gRobotIndexStartOffset +
                                                                   gNbOfRobots)   // sensor ray bumped into a robot : communication is possible
        {
            targetIndex = targetIndex - gRobotIndexStartOffset; // convert image registering index into robot id.

            TemplateEEController *targetRobotController = dynamic_cast<TemplateEEController *>(gWorld->getRobot(
                    targetIndex)->getController());
            ...
            {
                bool success = targetRobotController->storeGenome(_currentGenome,
                                                                  std::make_pair(_wm->getId(), _birthdate),
                                                                  _currentSigma,
                                                                  _wm->_fitnessValue); // other agent stores my genome. Contaminant stragegy. Note that medea does not use fitnessValue (default value: 0)
...
void TemplateEEController::performSelection() // called only if at least 1 genome was stored.
{
    switch (TemplateEESharedData::gSelectionMethod)
    {
        ...
        case 1:
            ...
            break;
        default:
            std::cerr << "[ERROR] unknown selection method (gSelectionMethod = "
                      << TemplateEESharedData::gSelectionMethod << ")\n";
            exit(-1);
    }
}
...
void TemplateEEController::loadNewGenome()
{
    if (_wm->isAlive() || gEnergyRefill)  // ( gEnergyRefill == true ) enables revive
    {
        ...
        if (_genomesList.size() > 0)
        {
            ...
            std::string sLog = std::string("");
            sLog += "" + std::to_string(gWorld->getIterations()) + "," + std::to_string(_wm->getId()) + "::" +
                    std::to_string(_birthdate) + ",status,active\n";
            gLogManager->write(sLog);
            gLogManager->flush();
