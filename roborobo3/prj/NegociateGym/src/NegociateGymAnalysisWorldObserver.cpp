...
NegociateGymAnalysisWorldObserver::NegociateGymAnalysisWorldObserver(World *__world) : WorldObserver(__world)
{
...
void NegociateGymAnalysisWorldObserver::reset()
{
    const double maxCoop = NegociateGymSharedData::maxCoop;
    int gen = 0;
    gProperties.checkAndGetPropertyValue("genAnalysis", &gen, true);
    ...
    {
        m_log << std::flush;
        auto *ctl = dynamic_cast<NegociateGymController *>(gWorld->getRobot(0)->getController());
        auto *wm = dynamic_cast<NegociateGymWorldModel *>(gWorld->getRobot(0)->getWorldModel());
