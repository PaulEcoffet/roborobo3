...
CoopOpportunity2MaxWorldObserver::CoopOpportunity2MaxWorldObserver(World *world) : WorldObserver(world)
{
    ...
    gProperties.checkAndGetPropertyValue("gEnergyRequestOutput", &CoopOpportunity2MaxSharedData::gEnergyRequestOutput,
                                         false);
    ...
    gProperties.checkAndGetPropertyValue("gNbNeuronsPerHiddenLayer",
                                         &CoopOpportunity2MaxSharedData::gNbNeuronsPerHiddenLayer, true);
    gProperties.checkAndGetPropertyValue("gNeuronWeightRange", &CoopOpportunity2MaxSharedData::gNeuronWeightRange,
                                         true);

    gProperties.checkAndGetPropertyValue("gSnapshots", &CoopOpportunity2MaxSharedData::gSnapshots, false);
    gProperties.checkAndGetPropertyValue("gSnapshotsFrequency", &CoopOpportunity2MaxSharedData::gSnapshotsFrequency,
                                         false);

    gProperties.checkAndGetPropertyValue("gControllerType", &CoopOpportunity2MaxSharedData::gControllerType, true);

    gProperties.checkAndGetPropertyValue("gIndividualMutationRate",
                                         &CoopOpportunity2MaxSharedData::gIndividualMutationRate, false);
    ...
    gProperties.checkAndGetPropertyValue("gLogGenomeSnapshot", &CoopOpportunity2MaxSharedData::gLogGenomeSnapshot,
                                         false);
...
void CoopOpportunity2MaxWorldObserver::updateMonitoring()
{
    if ((_generationCount + 1) % CoopOpportunity2MaxSharedData::gGenerationLog == 0 &&
        CoopOpportunity2MaxSharedData::gTakeVideo)
    {
...
void CoopOpportunity2MaxWorldObserver::monitorPopulation(bool localVerbose)
{
    ...
    for (int iRobot = 0; iRobot < nbTrueRobots; iRobot++)
    {
        CoopOpportunity2MaxController *ctl = dynamic_cast<CoopOpportunity2MaxController *>(gWorld->getRobot(
                iRobot)->getController());
        fitnesses[iRobot] = ctl->getFitness();
        index[iRobot] = iRobot;
    }
...
//        printf("Robot #%.2d is %.2d with %.2lf\n", i, index[nbTrueRobots-i-1], fitnesses[index[nbTrueRobots-i-1]]);
    ...
    std::cout << "log," << (gWorld->getIterations() / CoopOpportunity2MaxSharedData::gEvaluationTime) << ","
