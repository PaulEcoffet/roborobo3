...
void TemplateBoidsController::step()
{
    ...
    double attractionRadius = gSensorRange *
                              0.8; // 0.7 // if ( attractionRadius < repulsiveRadius ) then: assume attractionRadius=repulsiveRadius
    ...
    for (int i = 0; i < _wm->_cameraSensorsNb; i++)
    {
        ...
        if (targetRawIndex >=
            gRobotIndexStartOffset)   // sensor ray bumped into a robot -- WARNING: one (very close) neighbor can be seen by more than one sensor (in the following, we dont care if this happen, though it is not correct to do so)
        {
            int targetRobotIndex =
                    targetRawIndex - gRobotIndexStartOffset; // convert image registering index into robot id.
        ...
        {
            if (targetRawIndex == 0 &&
                _wm->getCameraSensorValue(i, SENSOR_DISTANCEVALUE) < minDistToWall) // closest wall
    ...
    if (minDistToWall < wallAvoidanceMinDist) // (too) close to a wall?
    {
        ...
        else
        {
            ...
            _wm->_desiredTranslationalValue = +1 - ((double) gSensorRange -
                                                   ...
                                                   (double) gSensorRange;
            if (_wm->getCameraSensorValue(0, SENSOR_DISTANCEVALUE) +
                _wm->getCameraSensorValue(1, SENSOR_DISTANCEVALUE) +
                _wm->getCameraSensorValue(2, SENSOR_DISTANCEVALUE) <
                _wm->getCameraSensorValue(3, SENSOR_DISTANCEVALUE) +
                _wm->getCameraSensorValue(4, SENSOR_DISTANCEVALUE) + _wm->getCameraSensorValue(5, SENSOR_DISTANCEVALUE))
                _wm->_desiredRotationalVelocity = +5;
            else if (_wm->getCameraSensorValue(3, SENSOR_DISTANCEVALUE) +
                     _wm->getCameraSensorValue(4, SENSOR_DISTANCEVALUE) +
    ...
    else
    {
        ...
        else
        {
            if (minDist < repulsiveRadius)
            {
                ...
                double angleDeltaToCenterOfMass = getAngleToTarget(_wm->_xReal, _wm->_yReal,
                                                                   _wm->_agentAbsoluteOrientation, avgCenterOfMass_X,
                                                                   avgCenterOfMass_Y);
            ...
            else
            {
                if (minDist > attractionRadius)
                {
                    ...
                    double angleDeltaToCenterOfMass = getAngleToTarget(_wm->_xReal, _wm->_yReal,
                                                                       ...
                                                                       avgCenterOfMass_X, avgCenterOfMass_Y);
