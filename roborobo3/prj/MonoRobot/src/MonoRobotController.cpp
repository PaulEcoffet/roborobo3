...
void MonoRobotController::initController()
{
    ...
    {
        _currentGenome.push_back((double) (randint() % MonoRobotSharedData::gNeuronWeightRange) /
                                 (MonoRobotSharedData::gNeuronWeightRange / 2) -
...
void
MonoRobotController::step() // handles control decision and evolution (but: actual movement is done in roborobo's main loop)
{
...
{
    if (MonoRobotSharedData::gIndividualMutationRate >
        ...
        switch (MonoRobotSharedData::gMutationOperator)
        {
            ...
            default:
                std::cerr << "[ERROR] unknown variation method (gMutationOperator = "
                          << MonoRobotSharedData::gMutationOperator << ")\n";
                exit(-1);
        }
    }
}
...
void MonoRobotController::wasNearObject(int __objectId, bool __objectDidMove, double __totalEffort, double __effort,
                                        int __nbRobots)
{
