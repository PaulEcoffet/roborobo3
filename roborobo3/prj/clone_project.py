...
def cloneProject(source, target):
    ...
    print(
        "This script create a new directory in the prj/ directory, and also add/update files in <roborobo>/src and <roborobo>/include. This script may not work with all projects. It assumes the source project can be duplicated without creating conflicts (ie. prefix \"+source+\" is used everywhere, no fancy names (class names, macro, global variables)).")
    print(
        "Note: if everything goes wrong, clean <roborobo>/src/ext/ConfigurationLoader.cpp (optional: delete extra files created).")
    ...
    for subdir in subdirs:
        ...
        for oldname in filenames:
            ...
            if fileExtension in [".cpp", ".h", ".hpp"]:
                ...
                print(
                    "<roborobo>/prj/" + target + subdir + oldname + " ignored and deleted (unknown type).")  ###  CHANGED!!!
    ...
    print
    "  1. delete directory <roborobo>/" + target + ""
    print
    "  2. edit file " + configurationLoaderFilename + ""
    print
    "\tdelete line: \"#include \"Config/" + target + "ConfigurationLoader.h\""
    print
    "\tdelete block: \"#if define PRJ_" + target.upper() + "\" (...) #endif\""
    print
    "\tdelete files: \"" + target + "ConfigurationLoader.cpp/.h\" in <roborobo>/src/ext and <roborobo>/include/ext/Config"
    print("")
    print
    "To run this project:"
    print
    "  1. Create a configuration file in the config/ directory, it should contain the following line:"
    print
    "\t\"ConfigurationLoaderObjectName = " + target + "ConfigurationLoader\""
    print
    "  2. Launch roborobo:"
    print
    "\tCommand-line (Linux):"
    print
    "\t\tcd <roborobo_root_directory>"
    print
    "\t\t./makefile-manager -a " + target
    print
    "\t\t make clean ; make"
    print
    "\t\t\"./roborobo -l Config/<yourconfigurationfile>.properties\""
    print
    "\tXcode on Mac"
    print
    "\t\tAdd new files to project"
    print
    "\t\tIf necessary, add filenames to list of sources to compile"
    print
    "\t\t\t1. click on projet name (project navigator window, upper-left)"
    print
    "\t\t\t2. select roborobo3 as target, not project (project/target list window, drop-down list)"
    print
    "\t\t\t4. select the \'build phases\' tab"
    print
    "\t\t\t5. go to the \'compile sources\' list"
    print
    "\t\t\t6. add the new files, if needed"
    print
    "\t\tRun with new configuration file as argument"
    print
    "\t\t\tClick on project name (top bar), then \'edit schemes\'"
    print
    "\n"
    print
    ""
